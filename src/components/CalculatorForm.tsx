...
export const CalculatorForm = forwardRef<HTMLDivElement, CalculatorFormProps>(({
  ...
  return <section ref={ref} className="py-20 px-4" id="calculator">
        <div className="max-w-2xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="glass-card rounded-3xl p-8 md:p-12 space-y-8">
            ...
            <div className="space-y-2">
              ...
              <Input id="name" type="text" placeholder="Masukkan nama lengkapmu" value={name} onChange={e => setName(e.target.value)} className="bg-input border-border text-foreground placeholder:text-muted-foreground h-12 rounded-xl" required />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input id="birthDate" type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)} className="hd-date relative bg-input border-border text-foreground h-12 rounded-xl pr-10" required />
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input id="birthTime" type="time" value={birthTime} onChange={e => setBirthTime(e.target.value)} className="hd-time relative bg-input border-border text-foreground h-12 rounded-xl pr-10" required />
            ...
            <div className="space-y-2 relative">
              ...
              <div className="relative">
                ...
                <Input id="city" type="text" placeholder="Ketik nama kota..." value={citySearch} onChange={e => {
              setCitySearch(e.target.value);
              setSelectedCity(null);
            }} onFocus={() => citySuggestions.length > 0 && setShowSuggestions(true)} className="bg-input border-border text-foreground placeholder:text-muted-foreground h-12 rounded-xl pl-10" required />
                {isSearching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground animate-spin" />}
              </div>

              {/* Suggestions dropdown */}
              {showSuggestions && citySuggestions.length > 0 && <div className="absolute z-20 w-full mt-1 bg-card border border-border rounded-xl shadow-lg overflow-hidden">
                  {citySuggestions.map((city, index) => <button key={index} type="button" onClick={() => handleCitySelect(city)} className="w-full text-left px-4 py-3 hover:bg-secondary text-foreground text-sm transition-colors border-b border-border last:border-0">
                      {city.display_name}
                    </button>)}
                </div>}
            </div>
            ...
            {/* Submit Button */}
            <Button type="submit" size="lg" disabled={isLoading || !selectedCity} className="w-full fire-glow bg-primary hover:bg-primary/90 text-primary-foreground text-lg py-6 rounded-xl font-semibold disabled:opacity-50">
              {isLoading ? <>
